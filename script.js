...
const STORAGE_KEYS = {
  FAV: 'bibliomics_favs',
  HST: 'bibliomics_hist',
  RATING: 'bibliomics_ratings',
  USER: 'bibliomics_user'
};
...
async function loadData() {
  try {
    ...
    renderSections();
    renderNewComics(); // 👉 Ajout de la section "Nouveaux Comics"
    attachHandlers();
  } catch (e) {
    console.error('Erreur lecture comics.json', e);
  }
}
...
// ✅ NOUVELLE FONCTION : affiche les comics dans la section "Nouveaux Comics"
function renderNewComics() {
  const container = document.querySelector('.nouveaux-comics');
  if (!container) {
    console.warn('Section .nouveaux-comics non trouvée');
    return;
  }
  ...
  comics.slice(0, 4).forEach(comic => {
    ...
    card.innerHTML = `
      <img src="${comic.cover || comic.image || ''}" alt="${comic.title || comic.titre}" />
      <h3>${comic.title || comic.titre}</h3>
      <p><strong>${comic.auteur || comic.author || 'Auteur inconnu'}</strong></p>
      <p>${comic.description || comic.short || ''}</p>
    `;
...
function fillRow(elemId, items) {
  ...
  items.forEach(it => {
    ...
    card.className = 'card';
    card.innerHTML = `
      <div class="cover" style="background-image:url('${it.cover}')"></div>
      <div class="meta">
        <div class="title">${it.title}</div>
        <div class="desc">${it.short || ''}</div>
      </div>
    `;
...
function renderCategory(cat) {
  const list = comics.filter(c => c.category === cat || c.categorie === cat);
...
function openReader(id) {
  ...
  const reader = qs('#reader-area');
  reader.innerHTML = '';

  if (item.viewer && item.viewer.type === 'iframe') {
    const ifr = document.createElement('iframe');
    ifr.src = item.viewer.src;
    ifr.style.width = '100%';
    ...
    reader.innerHTML = '<div style="color:#fff">Aperçu non disponible. Utilisez "Télécharger".</div>';
  }

  qs('#download-btn').onclick = () => {
    if (item.download) window.open(item.download, '_blank');
    else alert('Pas de lien de téléchargement.');
  };
...
function toggleFav(id) {
  ...
  if (idx === -1) {
    favs.push(id);
    alert('Ajouté à Ma bibliothèque');
  } else {
    favs.splice(idx, 1);
    alert('Retiré de Ma bibliothèque');
  }
...
function renderRatingUI(id) {
  ...
  const container = qs('#rating-area');
  container.innerHTML = '';
